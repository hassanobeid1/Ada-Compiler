...
public abstract class AbstractExpr extends AbstractInst {
    ...
    public AbstractExpr verifyRValue(DecacCompiler compiler, EnvironmentExp localEnv, ClassDefinition currentClass,
        ...
        if (!type.isAssignCompatible(expectedType)) {
            throw new ContextualError("Le type de l'expression n'est pas compatible", this.getLocation());
        }
        if (expectedType.isFloat() && type.isInt()) {
            ConvFloat x = new ConvFloat(this);
            x.setType(compiler.getType(compiler.getSymbol("float")).getType());
            return x;
        } else {
            return this;
        }
    ...
    void verifyCondition(DecacCompiler compiler, EnvironmentExp localEnv, ClassDefinition currentClass)
        ...
        if (!this.verifyExpr(compiler, localEnv, currentClass).isBoolean()) {
            throw new ContextualError("L'expression n'est pas un boolean", this.getLocation());
        }
    }
    ...
    protected void codeGenInst(DecacCompiler compiler, GPRegister registre) {
        if (this.getType().isBoolean()) {
            Label debut_faux = compiler.getLabel("debut_faux");
            Label fin_faux = compiler.getLabel("fin_faux");
            this.codeGenBool(compiler, false, debut_faux);
            // Vrai
            compiler.addInstruction(new LOAD(new ImmediateInteger(1), registre));
            compiler.addInstruction(new BRA(fin_faux));
            // Faux
            compiler.addLabel(debut_faux);
            compiler.addInstruction(new LOAD(new ImmediateInteger(0), registre));
            compiler.addLabel(fin_faux);
        } else {
    ...
    protected void codeGenPrint(DecacCompiler compiler, boolean hex, GPRegister registre) {
        this.codeGenInst(compiler, registre);
        compiler.addInstruction(new LOAD(registre, Register.R1));
        ...
        if (type.isInt()) {
            ...
            throw new UnsupportedOperationException("Print n'est pas implementé pour ce type");
        }
    }
    ...
    protected void codeGenBool(DecacCompiler compiler, boolean val, Label label) {
        if (!this.getType().isBoolean()) {
            throw new UnsupportedOperationException(
                    "On ne peut pas generer du code boolean pour une variable non boolean");
        } else {
            throw new UnsupportedOperationException("not yet implemented");
        }
    }
