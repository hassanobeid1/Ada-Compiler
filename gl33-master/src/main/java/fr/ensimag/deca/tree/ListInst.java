...
public class ListInst extends TreeList<AbstractInst> {
     ...
     * @param returnType   corresponds to "return" attribute (void in the main
     *                     bloc).
     */
    ...
    public void codeGenListInst(DecacCompiler compiler) {
        for (AbstractInst i : getList()) {
            i.codeGenInst(compiler, compiler.getMinReg());
        }
    }
    ...
    public void decompile(IndentPrintStream s) {
        ...
        it.forEachRemaining(inst -> {
            inst.decompile(s);
            if(!(inst instanceof While || inst instanceof IfThenElse)) {
                s.print(";");
            }
            s.println();
        });
